...
class OrderSheetPage(BasePage):
    ...
    CONFIRM_TRANSFER_ITEMS_BUTTON_ROLE = (
        "button",
        {"name": "Transferir"},
    )  # Botão "Transferir" no diálogo de itens
    ...
    def receive_payment_for_table(self, table_num: int, tax_emit: int, payment_type: str = None):
        ...
        if self._is_order_sheet_opened():
            ...
            payment_methods = {
                "Cash": lambda: self.click('button[name="CASH"]'),  # pylint: disable=unnecessary-lambda
                "Pix": lambda: self.click('button[name="PIX"]'),  # pylint: disable=unnecessary-lambda
                "Debit": lambda: self.click('button[name="DEBIT"]'),  # pylint: disable=unnecessary-lambda
                "Credit": lambda: self.click('button[name="CREDIT"]'),  # pylint: disable=unnecessary-lambda
                "OnCustomerAccount": lambda: self.on_customer_account(),  # pylint: disable=unnecessary-lambda
                "MealVoucher": lambda: self.select_voucher_type(1),  # pylint: disable=unnecessary-lambda
                "MealGift": lambda: self.select_voucher_type(2),  # pylint: disable=unnecessary-lambda
                "Courtesy": lambda: self.select_voucher_type(3),  # pylint: disable=unnecessary-lambda
            }
            ...
            if tax_note_modal:
                self.page.locator(f"xpath=/html/body/div[9]/div/div/div/div/div/div[2]/div[2]/button[{tax_emit}]").click()  # Cancelar ou enviar a emissão da nota fiscal
