...
def test_pay_all_unpaid_orders(logged_in_page):
    ...
    while True:
        ...
        if not order_id:
            logging.info("🏁 Não há mais pedidos pendentes na lista.")
            break
    ...
    if orders_paid_count == 0:
        ...
        logging.info(f"🎉 Sucesso! Total de pedidos pagos neste teste: {orders_paid_count}")
...
def test_order_status_flow(logged_in_page):
    ...
    while True:
        ...
        if not order_id:
            logging.info("🏁 Não há mais pedidos 'PENDENTE' na lista.")
            break
        ...
        try:
            ...
            logging.info(f"✅ Pedido {order_id} finalizado e removido da lista visual.")
